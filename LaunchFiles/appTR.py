...
chat_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Audio(type="filepath", label="İsteğinizi kaydedin (isteğe bağlı)"),
        gr.Image(label="Resminizi yükleyin (isteğe bağlı)", type="filepath"),
        gr.Dropdown(choices=llm_models_list, label="LLM modelini seçin", value=None),
        gr.Dropdown(choices=llm_lora_models_list, label="LoRA modelini seçin (isteğe bağlı)", value=None),
        gr.HTML("<h3>LLM Ayarları</h3>"),
        gr.Radio(choices=["transformers", "llama"], label="Model türünü seçin", value="transformers"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="Maksimum uzunluk (transformers türü modeller için)"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="Maksimum token (llama türü modeller için)"),
        gr.Slider(minimum=0.0, maximum=2.0, value=0.7, step=0.1, label="Sıcaklık"),
        ...
        gr.Radio(choices=["txt", "json"], label="Sohbet geçmişi formatını seçin", value="txt", interactive=True),
        gr.Checkbox(label="WebSearch'ü Etkinleştir", value=False),
        gr.Checkbox(label="LibreTranslate'i Etkinleştir", value=False),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Hedef dili seçin", value="tr", interactive=True),
        gr.Checkbox(label="Multimodal'i Etkinleştir", value=False),
        gr.Checkbox(label="TTS'yi Etkinleştir", value=False),
        gr.HTML("<h3>TTS Ayarları</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="Sesi seçin", interactive=True),
        gr.Dropdown(choices=["en", "tr"], label="Dili seçin", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.9, value=1.0, step=0.1, label="TTS Sıcaklığı", interactive=True),
        ...
        gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="TTS Hızı", interactive=True),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin", value="wav", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Chatbot(label="LLM metin yanıtı", value=[]),
        gr.Audio(label="LLM ses yanıtı", type="filepath"),
    ],
    title="NeuroSandboxWebUI (ALFA) - LLM",
    description="Bu kullanıcı arayüzü, herhangi bir metin veya ses girmenize ve oluşturulan yanıtı almanıza olanak tanır. "
                "Açılır listelerden LLM modelini, avatarı, sesi ve tts için dili seçebilirsiniz. "
                "Ayrıca kaydırıcılardan model ayarlarını özelleştirebilirsiniz. "
...
tts_stt_interface = gr.Interface(
    fn=generate_tts_stt,
    inputs=[
        gr.Textbox(label="TTS için metin girin"),
        gr.Audio(label="STT için ses kaydedin", type="filepath"),
        gr.HTML("<h3>TTS Ayarları</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="Sesi seçin", interactive=True),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh-cn", "ja", "hu", "ko", "hi"], label="Dili seçin", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.9, value=1.0, step=0.1, label="TTS Sıcaklığı", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.9, step=0.1, label="TTS Top P", interactive=True),
        gr.Slider(minimum=0, maximum=100, value=20, step=1, label="TTS Top K", interactive=True),
        gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="TTS Hızı", interactive=True),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="TTS çıktı formatını seçin", value="wav", interactive=True),
        gr.Dropdown(choices=["txt", "json"], label="STT çıktı formatını seçin", value="txt", interactive=True),
    ],
    outputs=[
        gr.Audio(label="TTS Sesi", type="filepath"),
        gr.Textbox(label="STT Metni"),
    ],
    title="NeuroSandboxWebUI (ALFA) - TTS-STT",
    description="Bu kullanıcı arayüzü, Metin-Konuşma (CoquiTTS) için metin girmenize ve Konuşma-Metin (OpenAIWhisper) için ses kaydetmenize olanak tanır. "
                "TTS için sesi ve dili seçebilir, kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "STT için sadece sesinizi kaydedin ve konuşulan metin görüntülenecektir. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
bark_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İstek için metin girin"),
        gr.Dropdown(choices=[None, "v2/en_speaker_1", "v2/tr_speaker_1", "v2/de_speaker_1", "v2/fr_speaker_1", "v2/es_speaker_1", "v2/hi_speaker_1", "v2/it_speaker_1", "v2/ja_speaker_1", "v2/ko_speaker_1", "v2/pt_speaker_1", "v2/zh_speaker_1", "v2/pl_speaker_1"], label="Ses ön ayarını seçin", value=None),
        gr.Slider(minimum=1, maximum=1000, value=100, step=1, label="Maksimum uzunluk"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.4, step=0.1, label="İnce sıcaklık"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.8, step=0.1, label="Kaba sıcaklık"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin", value="wav", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Oluşturulan ses", type="filepath"),
        gr.Image(label="Mel-Spektrogramı", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - SunoBark",
    description="Bu kullanıcı arayüzü, metin girmenize ve SunoBark kullanarak ses oluşturmanıza olanak tanır. "
                "Ses ön ayarını seçebilir ve maksimum uzunluğu özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
translate_interface = gr.Interface(
    fn=translate_text,
    inputs=[
        gr.Textbox(label="Çevrilecek metni girin"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Kaynak dili seçin", value="en"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Hedef dili seçin", value="tr"),
        gr.Checkbox(label="Çeviri geçmişi kaydetmeyi etkinleştir", value=False),
        gr.Radio(choices=["txt", "json"], label="Çeviri geçmişi formatını seçin", value="txt", interactive=True),
        gr.File(label="Metin dosyası yükleyin (isteğe bağlı)", file_count="single", interactive=True),
    ],
    outputs=[
        gr.Textbox(label="Çevrilmiş metin"),
    ],
    title="NeuroSandboxWebUI (ALFA) - LibreTranslate",
    description="Bu kullanıcı arayüzü, metin girmenize ve LibreTranslate kullanarak çevirmenize olanak tanır. "
                "Kaynak ve hedef dilleri seçin ve çeviriyi almak için Gönder'e tıklayın. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
wav2lip_interface = gr.Interface(
    fn=generate_wav2lip,
    inputs=[
        gr.Image(label="Giriş resmi", type="filepath"),
        gr.Audio(label="Giriş sesi", type="filepath"),
        gr.Slider(minimum=1, maximum=60, value=30, step=1, label="FPS"),
        gr.Textbox(label="Dolgular", value="0 10 0 0"),
        gr.Slider(minimum=1, maximum=64, value=16, step=1, label="Yüz Algılama Parti Boyutu"),
        gr.Slider(minimum=1, maximum=512, value=128, step=1, label="Wav2Lip Parti Boyutu"),
        gr.Slider(minimum=1, maximum=4, value=1, step=1, label="Yeniden Boyutlandırma Faktörü"),
        gr.Textbox(label="Kırpma", value="0 -1 0 -1"),
    ],
    outputs=[
        gr.Video(label="Oluşturulan dudak senkronizasyonu"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - Wav2Lip",
    description="Bu kullanıcı arayüzü, Wav2Lip kullanarak bir resim ve ses dosyasını birleştirerek konuşan kafa videoları oluşturmanıza olanak tanır. "
                "Bir resim ve bir ses dosyası yükleyin ve konuşan kafa videosunu oluşturmak için Oluştur'a tıklayın. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin", value=None),
        gr.Dropdown(choices=vae_models_list, label="VAE modelini seçin (isteğe bağlı)", value=None),
        gr.Dropdown(choices=lora_models_list, label="LORA modellerini seçin (isteğe bağlı)", value=None, multiselect=True),
        gr.Dropdown(choices=textual_inversion_models_list, label="Gömme modellerini seçin (isteğe bağlı)", value=None, multiselect=True),
        gr.HTML("<h3>StableDiffusion Ayarları</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Model türünü seçin", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
                    label="Örnekleyici seçin", value="euler_ancestral"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=1, maximum=4, value=1, step=1, label="Clip atla"),
        gr.Checkbox(label="FreeU'yu Etkinleştir", value=False),
        gr.Checkbox(label="Döşemeli VAE'yi Etkinleştir", value=False),
        gr.Checkbox(label="Yükseltmeyi Etkinleştir", value=False),
        gr.Radio(choices=["x2", "x4"], label="Yükseltme boyutu", value="x2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Yükseltme adımları"),
        gr.Slider(minimum=1.0, maximum=30.0, value=6, step=0.1, label="Yükseltme CFG"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Oluşturulan görüntü"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (txt2img)",
    description="Bu kullanıcı arayüzü, herhangi bir metin girmenize ve StableDiffusion kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
img2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="Güç"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin", value=None),
        gr.Dropdown(choices=vae_models_list, label="VAE modelini seçin (isteğe bağlı)", value=None),
        gr.HTML("<h3>StableDiffusion Ayarları</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Model türünü seçin", value="SD"),
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (img2img)",
    description="Bu kullanıcı arayüzü, herhangi bir metin ve görüntü girmenize ve StableDiffusion kullanarak yeni görüntüler oluşturmanıza olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
depth2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.HTML("<h3>StableDiffusion Ayarları</h3>"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.7, step=0.01, label="Güç"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (depth2img)",
    description="Bu kullanıcı arayüzü, bir istek girmenize, StableDiffusion kullanarak derinlik farkında görüntüler oluşturmak için bir başlangıç görüntüsü yüklemenize olanak tanır. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
pix2pix_interface = gr.Interface(
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (pix2pix)",
    description="Bu kullanıcı arayüzü, bir istek ve bir başlangıç görüntüsü girmenize ve Pix2Pix kullanarak yeni görüntüler oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.Radio(choices=["SD", "SDXL"], label="Model türünü seçin", value="SD"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin", value=None),
        gr.Dropdown(choices=controlnet_models_list, label="ControlNet modelini seçin", value=None),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="ControlNet koşullandırma ölçeği"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Oluşturulan görüntü"),
        gr.Image(type="filepath", label="ControlNet kontrol görüntüsü"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (controlnet)",
    description="Bu kullanıcı arayüzü, ControlNet modelleri kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir başlangıç görüntüsü yükleyin, bir istek girin, bir Stable Diffusion modeli seçin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
latent_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Yükseltilecek görüntü", type="filepath"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Yükseltilmiş görüntü"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (upscale-latent)",
    description="Bu kullanıcı arayüzü, bir görüntü yüklemenize ve onu gizli olarak yükseltmenize olanak tanır",
    allow_flagging="never",
)
...
realesrgan_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Yükseltilecek görüntü", type="filepath"),
        gr.Slider(minimum=0.1, maximum=8, value=4, step=0.1, label="Yükseltme faktörü"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (upscale-realesrgan)",
    description="Bu kullanıcı arayüzü, bir görüntü yüklemenize ve onu Real-ESRGAN kullanarak yükseltmenize olanak tanır",
    allow_flagging="never",
)
...
inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.ImageEditor(label="Maske görüntüsü", type="filepath"),
        gr.Slider(minimum=0, maximum=100, value=0, step=1, label="Maske Bulanıklık Faktörü"),
        gr.Dropdown(choices=inpaint_models_list, label="Inpaint modelini seçin", value=None),
        gr.Dropdown(choices=vae_models_list, label="VAE modelini seçin (isteğe bağlı)", value=None),
        gr.HTML("<h3>StableDiffusion Ayarları</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Model türünü seçin", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    outputs=[
        gr.Image(type="filepath", label="Boyanan görüntü"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (inpaint)",
    description="Bu kullanıcı arayüzü, bir istek girmenize, StableDiffusion kullanarak boyama yapmak için bir başlangıç görüntüsü ve bir maske görüntüsü yüklemenize olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
gligen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Textbox(label="GLIGEN ifadelerini girin", value=""),
        gr.Textbox(label="GLIGEN kutularını girin", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin", value=None),
        gr.HTML("<h3>StableDiffusion Ayarları</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Model türünü seçin", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (gligen)",
    description="Bu kullanıcı arayüzü, Stable Diffusion kullanarak görüntüler oluşturmanıza ve GLIGEN kullanarak nesneler eklemenize olanak tanır. "
                "Stable Diffusion modelini seçin, oluşturma ayarlarını özelleştirin, bir istek, GLIGEN ifadeleri ve sınırlayıcı kutular girin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
animatediff_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç GIF'i", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="Güç"),
        gr.Radio(choices=["sd", "sdxl"], label="Model türünü seçin", value="sd"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin (sadece SD1.5)", value=None),
        gr.Dropdown(choices=[None, "zoom-in", "zoom-out", "tilt-up", "tilt-down", "pan-right", "pan-left"], label="Hareket LORA'sını seçin", value=None, multiselect=True),
        gr.Slider(minimum=1, maximum=200, value=20, step=1, label="Kareler"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Yükseklik"),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(label="Oluşturulan GIF", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (animatediff)",
    description="Bu kullanıcı arayüzü, bir istek girmenize ve AnimateDiff kullanarak animasyonlu GIF'ler oluşturmanıza olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
video_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.Radio(choices=["mp4", "gif"], label="Çıktı formatını seçin", value="mp4", interactive=True),
        gr.HTML("<h3>SVD Ayarları (mp4)</h3>"),
        gr.Slider(minimum=0, maximum=360, value=180, step=1, label="Hareket Kova ID'si"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.1, step=0.01, label="Gürültü Artırma Gücü"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="FPS"),
        gr.Slider(minimum=2, maximum=120, value=25, step=1, label="Kareler"),
        gr.Slider(minimum=1, maximum=32, value=8, step=1, label="Çözme Parça Boyutu"),
        gr.HTML("<h3>I2VGen-xl Ayarları (gif)</h3>"),
        gr.Textbox(label="İstek", value=""),
        gr.Textbox(label="Negatif İstek", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=9.0, step=0.1, label="CFG"),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Oluşturulan video"),
        gr.Image(label="Oluşturulan GIF", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (video)",
    description="Bu kullanıcı arayüzü, bir başlangıç görüntüsü girmenize ve StableVideoDiffusion(mp4) ve I2VGen-xl(gif) kullanarak video oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
ldm3d_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="Rehberlik Ölçeği"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Oluşturulan RGB görüntüsü"),
        gr.Image(type="filepath", label="Oluşturulan Derinlik görüntüsü"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (LDM3D)",
    description="Bu kullanıcı arayüzü, bir istek girmenize ve LDM3D kullanarak RGB ve Derinlik görüntüleri oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
sd3_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Yükseklik"),
        gr.Slider(minimum=64, maximum=2048, value=256, label="Maksimum Uzunluk"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion 3 (txt2img)",
    description="Bu kullanıcı arayüzü, herhangi bir metin girmenize ve Stable Diffusion 3 kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
sd3_img2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Güç"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Yükseklik"),
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion 3 (img2img)",
    description="Bu kullanıcı arayüzü, herhangi bir metin ve başlangıç görüntüsü girmenize ve Stable Diffusion 3 kullanarak yeni görüntüler oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
sd3_controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.Dropdown(choices=["Pose", "Canny"], label="ControlNet modelini seçin", value="Pose"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="ControlNet koşullandırma ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Genişlik"),
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion 3 (ControlNet)",
    description="Bu kullanıcı arayüzü, Stable Diffusion 3 ile ControlNet modellerini kullanmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
sd3_inpaint_interface = gr.Interface(
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion 3 (Inpaint)",
    description="Bu kullanıcı arayüzü, Stable Diffusion 3 kullanarak boyama yapmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
cascade_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.HTML("<h3>Stable Cascade Ayarları</h3>"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="Yükseklik"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Öncül Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=4.0, step=0.1, label="Öncül Rehberlik Ölçeği"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="Çözücü Adımları"),
        gr.Slider(minimum=0.0, maximum=30.0, value=8.0, step=0.1, label="Çözücü Rehberlik Ölçeği"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (cascade)",
    description="Bu kullanıcı arayüzü, bir istek girmenize ve Stable Cascade kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
instantid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Yüz görüntüsü", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion XL modelini seçin", value=None),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="ControlNet koşullandırma ölçeği"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="IP-Adapter ölçeği"),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Rehberlik Ölçeği (CFG)"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - InstantID",
    description="Bu kullanıcı arayüzü, InstantID kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir yüz görüntüsü yükleyin, bir istek girin, bir Stable Diffusion modeli seçin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
photomaker_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.File(label="Giriş görüntülerini yükleyin", file_count="multiple", type="filepath"),
        gr.Textbox(label="Tetikleyici kelime girin", value="img"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion XL modelini seçin", value=None),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Rehberlik Ölçeği (CFG)"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
    ...
    title="NeuroSandboxWebUI (ALFA) - PhotoMaker",
    description="Bu kullanıcı arayüzü, PhotoMaker kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Giriş görüntüleri yükleyin, bir istek girin, bir temel model seçin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
ip_adapter_faceid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Image(label="Yüz görüntüsü yükleyin", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion modelini seçin", value=None),
        gr.Dropdown(
            choices=[
            ...
            label="IP-Adapter-FaceID versiyonunu seçin",
    ...
    title="NeuroSandboxWebUI (ALFA) - IP-Adapter-FaceID",
    description="Bu kullanıcı arayüzü, IP-Adapter-FaceID kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir yüz görüntüsü yükleyin, bir istek girin, bir temel model ve IP-Adapter versiyonu seçin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
extras_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Değiştirilecek görüntü", type="filepath"),
        gr.Image(label="Kaynak Görüntü", type="filepath"),
        gr.Checkbox(label="Arka Planı Kaldır", value=False),
        gr.Checkbox(label="Yüz Değiştirmeyi Etkinleştir", value=False),
        gr.Checkbox(label="Yüz Onarımını Etkinleştir", value=False),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(label="Değiştirilmiş görüntü", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableDiffusion (ekstralar)",
    description="Bu kullanıcı arayüzü, görüntüyü değiştirmenize olanak tanır",
    allow_flagging="never",
)
...
kandinsky_txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Radio(choices=["2.1", "2.2", "3"], label="Kandinsky Versiyonu", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Genişlik"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - Kandinsky (txt2img)",
    description="Bu kullanıcı arayüzü, Kandinsky modellerini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "2.1, 2.2 ve 3 versiyonları arasında seçim yapabilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
kandinsky_img2img_interface = gr.Interface(
    ...
    title="NeuroSandboxWebUI (ALFA) - Kandinsky (img2img)",
    description="Bu kullanıcı arayüzü, Kandinsky modellerini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "2.1, 2.2 ve 3 versiyonları arasında seçim yapabilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
...
kandinsky_inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="Başlangıç görüntüsü", type="filepath"),
        gr.ImageEditor(label="Maske görüntüsü", type="filepath"),
        gr.Radio(choices=["2.1", "2.2"], label="Kandinsky Versiyonu", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Güç"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
    ...
    title="NeuroSandboxWebUI (ALFA) - Kandinsky (inpaint)",
    description="Bu kullanıcı arayüzü, Kandinsky modellerini kullanarak boyama yapmanıza olanak tanır. "
                "2.1 ve 2.2 versiyonları arasında seçim yapabilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
flux_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Dropdown(choices=["FLUX.1-schnell", "FLUX.1-dev"], label="Flux modelini seçin", value="FLUX.1-schnell"),
        gr.Slider(minimum=0.0, maximum=10.0, value=0.0, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Genişlik"),
        gr.Slider(minimum=1, maximum=100, value=10, step=1, label="Adımlar"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksimum Sıralı Uzunluk"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - Flux",
    description="Bu kullanıcı arayüzü, Flux modellerini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Schnell ve Dev modelleri arasında seçim yapabilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
hunyuandit_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Genişlik"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ...
    title="NeuroSandboxWebUI (ALFA) - HunyuanDiT",
    description="Bu kullanıcı arayüzü, HunyuanDiT modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin (İngilizce veya Çince) ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
lumina_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Genişlik"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksimum Sıralı Uzunluk"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
    ...
    title="NeuroSandboxWebUI (ALFA) - Lumina-T2X",
    description="Bu kullanıcı arayüzü, Lumina-T2X modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
kolors_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.5, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=1, maximum=100, value=25, step=1, label="Adımlar"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksimum Sıralı Uzunluk"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    ...
    title="NeuroSandboxWebUI (ALFA) - Kolors",
    description="Bu kullanıcı arayüzü, Kolors modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
auraflow_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1, maximum=100, value=25, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksimum Sıralı Uzunluk"),
        gr.Checkbox(label="AuraSR'yi Etkinleştir", value=False),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - AuraFlow",
    description="Bu kullanıcı arayüzü, AuraFlow modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Ayrıca oluşturulan görüntünün 4x yükseltilmesi için AuraSR'yi etkinleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
wurstchen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=256, maximum=2048, value=1536, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Yükseklik"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Öncül Adımlar"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4.0, step=0.1, label="Öncül Rehberlik Ölçeği"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="Çözücü Adımları"),
        gr.Slider(minimum=0.0, maximum=30.0, value=0.0, step=0.1, label="Çözücü Rehberlik Ölçeği"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - Würstchen",
    description="Bu kullanıcı arayüzü, Würstchen modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
deepfloyd_if_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=30.0, value=6, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ...
    outputs=[
        gr.Image(type="filepath", label="Oluşturulan görüntü (Aşama I)"),
        gr.Image(type="filepath", label="Oluşturulan görüntü (Aşama II)"),
        gr.Image(type="filepath", label="Oluşturulan görüntü (Aşama III)"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - DeepFloyd IF (txt2img)",
    description="Bu kullanıcı arayüzü, DeepFloyd IF modelini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Süreç, her biri artan kalitede bir görüntü üreten üç oluşturma aşamasını içerir. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
deepfloyd_if_img2img_interface = gr.Interface(
    ...
    title="NeuroSandboxWebUI (ALFA) - DeepFloyd IF (img2img)",
    description="Bu arayüz, DeepFloyd IF'nin görüntüden görüntüye boru hattını kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Bir istek girin, bir başlangıç görüntüsü yükleyin ve oluşturma ayarlarını özelleştirin. "
                "Süreç, her biri artan kalitede bir görüntü üreten üç oluşturma aşamasını içerir. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
deepfloyd_if_inpaint_interface = gr.Interface(
    ...
    title="NeuroSandboxWebUI (ALFA) - DeepFloyd IF (inpaint)",
    description="Bu arayüz, DeepFloyd IF kullanarak boyama yapmanıza olanak tanır. "
                "Bir istek girin, bir başlangıç görüntüsü ve bir maske görüntüsü yükleyin ve oluşturma ayarlarını özelleştirin. "
                "Süreç, her biri artan kalitede bir görüntü üreten üç oluşturma aşamasını içerir. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
pixart_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Radio(choices=["Alpha-512", "Alpha-1024", "Sigma-512", "Sigma-1024"], label="PixArt Versiyonu", value="Alpha-512"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Genişlik"),
    ...
    title="NeuroSandboxWebUI (ALFA) - PixArt",
    description="Bu kullanıcı arayüzü, PixArt modellerini kullanarak görüntüler oluşturmanıza olanak tanır. "
                "Alpha ve Sigma versiyonları arasında seçim yapabilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
modelscope_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=1024, value=320, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=1024, value=576, step=64, label="Genişlik"),
        gr.Slider(minimum=16, maximum=128, value=64, step=1, label="Kare Sayısı"),
        gr.Radio(choices=["mp4", "gif"], label="Çıktı formatını seçin", value="mp4", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - ModelScope",
    description="Bu kullanıcı arayüzü, ModelScope kullanarak videolar oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
zeroscope2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Video(label="Geliştirilecek video (isteğe bağlı)", interactive=True),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="Güç"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Adımlar"),
        gr.Slider(minimum=256, maximum=1280, value=576, step=64, label="Genişlik"),
        gr.Slider(minimum=256, maximum=1280, value=320, step=64, label="Yükseklik"),
        gr.Slider(minimum=1, maximum=100, value=36, step=1, label="Kareler"),
        gr.Checkbox(label="Video Geliştirmeyi Etkinleştir", value=False),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Oluşturulan video"),
    ...
    title="NeuroSandboxWebUI (ALFA) - ZeroScope 2",
    description="Bu kullanıcı arayüzü, ZeroScope 2 modellerini kullanarak video oluşturmanıza ve geliştirmenize olanak tanır. "
                "Bir metin istemi girebilir, isteğe bağlı olarak geliştirme için bir video yükleyebilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
cogvideox_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.0, step=0.1, label="Rehberlik Ölçeği"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="Kare Sayısı"),
    ...
    title="NeuroSandboxWebUI (ALFA) - CogVideoX",
    description="Bu kullanıcı arayüzü, CogVideoX kullanarak videolar oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
latte_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Yükseklik"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Genişlik"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="Video Uzunluğu"),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Oluşturulan GIF"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - Latte",
    description="Bu kullanıcı arayüzü, Latte kullanarak GIF'ler oluşturmanıza olanak tanır. "
                "Bir istek girin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
triposr_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Giriş görüntüsü", type="pil"),
        gr.Slider(minimum=32, maximum=320, value=256, step=32, label="Marching Cubes Çözünürlüğü"),
        gr.Slider(minimum=0.5, maximum=1.0, value=0.85, step=0.05, label="Ön Plan Oranı"),
        gr.Radio(choices=["obj", "glb"], label="Çıktı formatını seçin", value="obj", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="Oluşturulan 3D nesne"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - TripoSR",
    description="Bu kullanıcı arayüzü, TripoSR kullanarak 3D nesneler oluşturmanıza olanak tanır. "
                "Bir görüntü yükleyin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
stablefast3d_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Giriş görüntüsü", type="filepath"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=256, label="Doku Çözünürlüğü"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.85, step=0.05, label="Ön Plan Oranı"),
        gr.Radio(choices=["none", "triangle", "quad"], label="Yeniden Ağ Seçeneği", value="none"),
        gr.Radio(choices=["obj", "glb"], label="Çıktı formatını seçin", value="obj", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - StableFast3D",
    description="Bu kullanıcı arayüzü, StableFast3D kullanarak görüntülerden 3D nesneler oluşturmanıza olanak tanır. "
                "Bir görüntü yükleyin ve oluşturma ayarlarını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
shap_e_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Image(label="Başlangıç görüntüsü (isteğe bağlı)", type="filepath", interactive=True),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Adımlar"),
        gr.Slider(minimum=1.0, maximum=30.0, value=10.0, step=0.1, label="CFG"),
        gr.Slider(minimum=64, maximum=512, value=256, step=64, label="Kare boyutu"),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="Oluşturulan 3D nesne"),
    ...
    title="NeuroSandboxWebUI (ALFA) - Shap-E",
    description="Bu kullanıcı arayüzü, Shap-E kullanarak 3D nesneler oluşturmanıza olanak tanır. "
                "Bir metin istemi girebilir veya bir başlangıç görüntüsü yükleyebilir ve oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
sv34d_interface = gr.Interface(
    fn=generate_sv34d,
    inputs=[
        gr.File(label="Giriş dosyası (3D-U ve 3D-P için Görüntü, 4D için MP4 video)", type="filepath"),
        gr.Radio(choices=["3D-U", "3D-P", "4D"], label="Versiyon", value="3D-U"),
        gr.Slider(minimum=0.0, maximum=90.0, value=10.0, step=0.1, label="Yükseklik Derecesi (sadece 3D-P için)"),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Oluşturulan çıktı"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - SV34D",
    description="Bu arayüz, SV34D modellerini kullanarak 3D ve 4D içerik oluşturmanıza olanak tanır. "
                "3D-U ve 3D-P versiyonları için bir görüntü (PNG, JPG, JPEG), 4D versiyonu için bir MP4 video yükleyin. "
                "Versiyonu seçin ve gerektiğinde ayarları özelleştirin.",
    allow_flagging="never",
)
...
zero123plus_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Giriş görüntüsü", type="filepath"),
        gr.Slider(minimum=1, maximum=100, value=75, step=1, label="Çıkarım adımları"),
        gr.Radio(choices=["png", "jpeg"], label="Çıktı formatını seçin", value="png", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    title="NeuroSandboxWebUI (ALFA) - Zero123Plus",
    description="Bu kullanıcı arayüzü, Zero123Plus kullanarak 3D benzeri görüntüler oluşturmanıza olanak tanır. "
                "Bir giriş görüntüsü yükleyin ve çıkarım adımlarının sayısını özelleştirin. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
stableaudio_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="Adımlar"),
        gr.Slider(minimum=0.1, maximum=12, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="Ses Uzunluğu (saniye)"),
        gr.Slider(minimum=1, maximum=60, value=0, step=1, label="Ses Başlangıcı (saniye)"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="Dalga Formu Sayısı"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin", value="wav", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Oluşturulan ses", type="filepath"),
        gr.Image(label="Mel-Spektrogram", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - StableAudio",
    description="Bu kullanıcı arayüzü, herhangi bir metin girmenize ve StableAudio kullanarak ses oluşturmanıza olanak tanır. "
                "Kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
audiocraft_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Audio(type="filepath", label="Melodi sesi (isteğe bağlı)", interactive=True),
        gr.Dropdown(choices=audiocraft_models_list, label="AudioCraft modelini seçin", value=None),
        gr.HTML("<h3>AudioCraft Ayarları</h3>"),
        gr.Radio(choices=["musicgen", "audiogen", "magnet"], label="Model türünü seçin", value="musicgen"),
        gr.Slider(minimum=1, maximum=120, value=10, step=1, label="Süre (saniye)"),
        ...
        gr.Slider(minimum=0.0, maximum=1.9, value=1.0, step=0.1, label="Sıcaklık"),
        gr.Slider(minimum=1.0, maximum=10.0, value=3.0, step=0.1, label="CFG"),
        gr.Checkbox(label="Çok Bantlı Difüzyonu Etkinleştir", value=False),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin (Sadece Çok Bantlı Difüzyon olmadan çalışır)", value="wav", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Oluşturulan ses", type="filepath"),
    ...
    title="NeuroSandboxWebUI (ALFA) - AudioCraft",
    description="Bu kullanıcı arayüzü, herhangi bir metin girmenize ve AudioCraft kullanarak ses oluşturmanıza olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
audioldm2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="İsteğinizi girin"),
        gr.Textbox(label="Negatif isteğinizi girin", value=""),
        gr.Dropdown(choices=["cvssp/audioldm2", "cvssp/audioldm2-music"], label="AudioLDM 2 modelini seçin", value="cvssp/audioldm2"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="Adımlar"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="Uzunluk (saniye)"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="Dalga formu sayısı"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin", value="wav", interactive=True),
        gr.Button(value="Oluşturmayı durdur", interactive=True, variant="stop"),
    ],
    ...
    title="NeuroSandboxWebUI (ALFA) - AudioLDM 2",
    description="Bu kullanıcı arayüzü, herhangi bir metin girmenize ve AudioLDM 2 kullanarak ses oluşturmanıza olanak tanır. "
                "Modeli seçebilir ve kaydırıcılardan oluşturma ayarlarını özelleştirebilirsiniz. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
demucs_interface = gr.Interface(
    fn=demucs_separate,
    inputs=[
        gr.Audio(type="filepath", label="Ayrılacak ses dosyası"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Çıktı formatını seçin", value="wav", interactive=True),
    ],
    outputs=[
        gr.Audio(label="Vokal", type="filepath"),
        gr.Audio(label="Enstrümantal", type="filepath"),
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - Demucs",
    description="Bu kullanıcı arayüzü, bir ses dosyası yüklemenize ve Demucs kullanarak vokal ve enstrümantal olarak ayırmanıza olanak tanır. "
                "Deneyin ve ne olacağını görün!",
    allow_flagging="never",
)
...
gallery_interface = gr.Interface(
    ...
    inputs=[
        gr.Dropdown(label="Metin Dosyaları", choices=get_output_files()[0], interactive=True),
        gr.Dropdown(label="Görüntü Dosyaları", choices=get_output_files()[1], interactive=True),
        gr.Dropdown(label="Video Dosyaları", choices=get_output_files()[2], interactive=True),
        gr.Dropdown(label="Ses Dosyaları", choices=get_output_files()[3], interactive=True),
        gr.Dropdown(label="3D Model Dosyaları", choices=get_output_files()[4], interactive=True),
    ],
    outputs=[
        gr.Textbox(label="Metin"),
        gr.Image(label="Görüntü", type="filepath"),
        gr.Video(label="Video"),
        gr.Audio(label="Ses", type="filepath"),
        gr.Model3D(label="3D Model"),
    ],
    title="NeuroSandboxWebUI (ALFA) - Galeri",
    description="Bu arayüz, çıktılar dizinindeki dosyaları görüntülemenize olanak tanır",
    allow_flagging="never",
)
...
model_downloader_interface = gr.Interface(
    fn=download_model,
    inputs=[
        gr.Dropdown(choices=[None, "StarlingLM(Transformers7B)", "OpenChat(Llama7B.Q4)"], label="LLM modelini indir", value=None),
        gr.Dropdown(choices=[None, "Dreamshaper8(SD1.5)", "RealisticVisionV4.0(SDXL)"], label="StableDiffusion modelini indir", value=None),
    ],
    outputs=[
        gr.Textbox(label="Mesaj", type="text"),
    ],
    title="NeuroSandboxWebUI (ALFA) - ModelDownloader",
    description="Bu kullanıcı arayüzü, LLM ve StableDiffusion modellerini indirmenize olanak tanır",
    allow_flagging="never",
)
...
settings_interface = gr.Interface(
    ...
    inputs=[
        gr.Radio(choices=["True", "False"], label="Paylaşım Modu", value="False")
    ],
    outputs=[
        gr.Textbox(label="Mesaj", type="text")
    ],
    title="NeuroSandboxWebUI (ALFA) - Ayarlar",
    description="Bu kullanıcı arayüzü, uygulamanın ayarlarını değiştirmenize olanak tanır",
    allow_flagging="never",
)
...
system_interface = gr.Interface(
    ...
    inputs=[],
    outputs=[
        gr.Textbox(label="GPU Toplam Bellek"),
        gr.Textbox(label="GPU Kullanılan Bellek"),
        gr.Textbox(label="GPU Boş Bellek"),
        gr.Textbox(label="GPU Sıcaklığı"),
        gr.Textbox(label="CPU Sıcaklığı"),
        gr.Textbox(label="RAM Toplam"),
        gr.Textbox(label="RAM Kullanılan"),
        gr.Textbox(label="RAM Boş"),
    ],
    title="NeuroSandboxWebUI (ALFA) - Sistem",
    description="Bu arayüz sistem bilgilerini görüntüler",
    allow_flagging="never",
)
...
with gr.TabbedInterface(
    [
        ...
        gr.TabbedInterface(
            ...
            tab_names=["Galeri", "ModelDownloader", "Ayarlar", "Sistem"]
        )
    ],
    tab_names=["Metin", "Görüntü", "Video", "3D", "Ses", "Arayüz"]
) as app:
    ...
    close_button = gr.Button("Terminali kapat")
    close_button.click(close_terminal, [], [], queue=False)

    folder_button = gr.Button("Çıktılar")
