...
chat_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的请求"),
        gr.Audio(type="filepath", label="录制您的请求（可选）"),
        gr.Image(label="上传您的图片（可选）", type="filepath"),
        gr.Dropdown(choices=llm_models_list, label="选择LLM模型", value=None),
        gr.Dropdown(choices=llm_lora_models_list, label="选择LoRA模型（可选）", value=None),
        gr.HTML("<h3>LLM设置</h3>"),
        gr.Radio(choices=["transformers", "llama"], label="选择模型类型", value="transformers"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="最大长度（用于transformers类型模型）"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="最大令牌数（用于llama类型模型）"),
        ...
        gr.Radio(choices=["txt", "json"], label="选择聊天历史格式", value="txt", interactive=True),
        gr.Checkbox(label="启用网络搜索", value=False),
        gr.Checkbox(label="启用LibreTranslate", value=False),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="选择目标语言", value="ru", interactive=True),
        gr.Checkbox(label="启用多模态", value=False),
        gr.Checkbox(label="启用TTS", value=False),
        gr.HTML("<h3>TTS设置</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="选择声音", interactive=True),
        gr.Dropdown(choices=["en", "ru"], label="选择语言", interactive=True),
        ...
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式", value="wav", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Chatbot(label="LLM文本响应", value=[]),
        gr.Audio(label="LLM音频响应", type="filepath"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - LLM",
    description="此用户界面允许您输入任何文本或音频并接收生成的响应。您可以从下拉列表中选择LLM模型、头像、语音和TTS语言。您还可以使用滑块自定义模型设置。尝试一下，看看会发生什么！",
...
tts_stt_interface = gr.Interface(
    fn=generate_tts_stt,
    inputs=[
        gr.Textbox(label="输入TTS文本"),
        gr.Audio(label="为STT录制音频", type="filepath"),
        gr.HTML("<h3>TTS设置</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="选择声音", interactive=True),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh-cn", "ja", "hu", "ko", "hi"], label="选择语言", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.9, value=1.0, step=0.1, label="TTS温度", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.9, step=0.1, label="TTS Top P", interactive=True),
        gr.Slider(minimum=0, maximum=100, value=20, step=1, label="TTS Top K", interactive=True),
        gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="TTS速度", interactive=True),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择TTS输出格式", value="wav", interactive=True),
        gr.Dropdown(choices=["txt", "json"], label="选择STT输出格式", value="txt", interactive=True),
    ],
    outputs=[
        gr.Audio(label="TTS音频", type="filepath"),
        gr.Textbox(label="STT文本"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - TTS-STT",
    description="此用户界面允许您输入文本进行文本到语音转换（CoquiTTS）和录制音频进行语音到文本转换（OpenAIWhisper）。对于TTS，您可以选择声音和语言，并使用滑块自定义生成设置。对于STT，只需录制您的音频，就会显示出口语文本。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
bark_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入请求文本"),
        gr.Dropdown(choices=[None, "v2/en_speaker_1", "v2/ru_speaker_1", "v2/de_speaker_1", "v2/fr_speaker_1", "v2/es_speaker_1", "v2/hi_speaker_1", "v2/it_speaker_1", "v2/ja_speaker_1", "v2/ko_speaker_1", "v2/pt_speaker_1", "v2/zh_speaker_1", "v2/tr_speaker_1", "v2/pl_speaker_1"], label="选择语音预设", value=None),
        gr.Slider(minimum=1, maximum=1000, value=100, step=1, label="最大长度"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.4, step=0.1, label="精细温度"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.8, step=0.1, label="粗略温度"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式", value="wav", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="生成的音频", type="filepath"),
        gr.Image(label="梅尔频谱图", type="filepath"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - SunoBark",
    description="此用户界面允许您输入文本并使用SunoBark生成音频。您可以选择语音预设并自定义最大长度。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
translate_interface = gr.Interface(
    fn=translate_text,
    inputs=[
        gr.Textbox(label="输入要翻译的文本"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="选择源语言", value="en"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="选择目标语言", value="ru"),
        gr.Checkbox(label="启用翻译历史保存", value=False),
        gr.Radio(choices=["txt", "json"], label="选择翻译历史格式", value="txt", interactive=True),
        gr.File(label="上传文本文件（可选）", file_count="single", interactive=True),
    ],
    outputs=[
        gr.Textbox(label="翻译后的文本"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - LibreTranslate",
    description="此用户界面允许您输入文本并使用LibreTranslate进行翻译。选择源语言和目标语言，然后点击提交获取翻译。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
wav2lip_interface = gr.Interface(
    fn=generate_wav2lip,
    inputs=[
        gr.Image(label="输入图像", type="filepath"),
        gr.Audio(label="输入音频", type="filepath"),
        gr.Slider(minimum=1, maximum=60, value=30, step=1, label="FPS"),
        gr.Textbox(label="填充", value="0 10 0 0"),
        gr.Slider(minimum=1, maximum=64, value=16, step=1, label="人脸检测批量大小"),
        gr.Slider(minimum=1, maximum=512, value=128, step=1, label="Wav2Lip批量大小"),
        gr.Slider(minimum=1, maximum=4, value=1, step=1, label="调整大小因子"),
        gr.Textbox(label="裁剪", value="0 -1 0 -1"),
    ],
    outputs=[
        gr.Video(label="生成的唇形同步视频"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Wav2Lip",
    description="此用户界面允许您使用Wav2Lip通过组合图像和音频文件生成说话头视频。上传图像和音频文件，然后点击生成创建说话头视频。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(choices=vae_models_list, label="选择VAE模型（可选）", value=None),
        gr.Dropdown(choices=lora_models_list, label="选择LORA模型（可选）", value=None, multiselect=True),
        gr.Dropdown(choices=textual_inversion_models_list, label="选择Embedding模型（可选）", value=None, multiselect=True),
        gr.HTML("<h3>StableDiffusion设置</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="选择模型类型", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
                    label="选择采样器", value="euler_ancestral"),
        ...
        gr.Checkbox(label="启用FreeU", value=False),
        gr.Checkbox(label="启用Tiled VAE", value=False),
        gr.Checkbox(label="启用上采样", value=False),
        gr.Radio(choices=["x2", "x4"], label="上采样大小", value="x2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="上采样步数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=6, step=0.1, label="上采样CFG"),
    ...
    description="此用户界面允许您输入任何文本并使用StableDiffusion生成图像。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
img2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="强度"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="选择StableDiffusion模型", value=None),
        gr.Dropdown(choices=vae_models_list, label="选择VAE模型（可选）", value=None),
        gr.HTML("<h3>StableDiffusion设置</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="选择模型类型", value="SD"),
    ...
    description="此用户界面允许您输入任何文本和图像，使用StableDiffusion生成新图像。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
depth2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="初始图像", type="filepath"),
        gr.HTML("<h3>StableDiffusion设置</h3>"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.7, step=0.01, label="强度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您输入提示和初始图像，使用StableDiffusion生成深度感知图像。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
pix2pix_interface = gr.Interface(
    ...
    description="此用户界面允许您输入提示和初始图像，使用Pix2Pix生成新图像。您可以使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
controlnet_interface = gr.Interface(
    ...
    outputs=[
        gr.Image(type="filepath", label="生成的图像"),
        gr.Image(type="filepath", label="ControlNet控制图像"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (controlnet)",
    description="此用户界面允许您使用ControlNet模型生成图像。上传初始图像，输入提示，选择Stable Diffusion模型，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
latent_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="要上采样的图像", type="filepath"),
    ...
    outputs=[
        gr.Image(type="filepath", label="上采样后的图像"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (upscale-latent)",
    description="此用户界面允许您上传图像并进行潜在空间上采样",
    allow_flagging="never",
)
...
realesrgan_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="要上采样的图像", type="filepath"),
        gr.Slider(minimum=0.1, maximum=8, value=4, step=0.1, label="上采样因子"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您上传图像并使用Real-ESRGAN进行上采样",
    allow_flagging="never",
)
...
inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Image(label="初始图像", type="filepath"),
        gr.ImageEditor(label="遮罩图像", type="filepath"),
        gr.Slider(minimum=0, maximum=100, value=0, step=1, label="遮罩模糊因子"),
        gr.Dropdown(choices=inpaint_models_list, label="选择修复模型", value=None),
        gr.Dropdown(choices=vae_models_list, label="选择VAE模型（可选）", value=None),
        gr.HTML("<h3>StableDiffusion设置</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="选择模型类型", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    outputs=[
        gr.Image(type="filepath", label="修复后的图像"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (inpaint)",
    description="此用户界面允许您输入提示、初始图像和遮罩图像，使用StableDiffusion进行图像修复。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
gligen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Textbox(label="输入GLIGEN短语", value=""),
        gr.Textbox(label="输入GLIGEN边界框", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="选择StableDiffusion模型", value=None),
        gr.HTML("<h3>StableDiffusion设置</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="选择模型类型", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    description="此用户界面允许您使用Stable Diffusion生成图像并使用GLIGEN插入对象。选择Stable Diffusion模型，自定义生成设置，输入提示、GLIGEN短语和边界框。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
animatediff_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Image(label="初始GIF", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="强度"),
        gr.Radio(choices=["sd", "sdxl"], label="选择模型类型", value="sd"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="选择StableDiffusion模型（仅限SD1.5）", value=None),
        gr.Dropdown(choices=[None, "zoom-in", "zoom-out", "tilt-up", "tilt-down", "pan-right", "pan-left"], label="选择动作LORA", value=None, multiselect=True),
        gr.Slider(minimum=1, maximum=200, value=20, step=1, label="帧数"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="步数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="引导尺度"),
    ...
    outputs=[
        gr.Image(label="生成的GIF", type="filepath"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (animatediff)",
    description="此用户界面允许您输入提示并使用AnimateDiff生成动画GIF。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
video_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="初始图像", type="filepath"),
        gr.Radio(choices=["mp4", "gif"], label="选择输出格式", value="mp4", interactive=True),
        gr.HTML("<h3>SVD设置（mp4）</h3>"),
        gr.Slider(minimum=0, maximum=360, value=180, step=1, label="动作桶ID"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.1, step=0.01, label="噪声增强强度"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="FPS"),
        gr.Slider(minimum=2, maximum=120, value=25, step=1, label="帧数"),
        gr.Slider(minimum=1, maximum=32, value=8, step=1, label="解码块大小"),
        gr.HTML("<h3>I2VGen-xl设置（gif）</h3>"),
        gr.Textbox(label="提示", value=""),
        gr.Textbox(label="负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
    ...
    description="此用户界面允许您输入初始图像并使用StableVideoDiffusion（mp4）和I2VGen-xl（gif）生成视频。您可以使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
ldm3d_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="高度"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="步数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="引导尺度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="生成的RGB图像"),
        gr.Image(type="filepath", label="生成的深度图像"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (LDM3D)",
    description="此用户界面允许您输入提示并使用LDM3D生成RGB和深度图像。您可以使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
sd3_txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="步骤"),
        ...
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="高度"),
        gr.Slider(minimum=64, maximum=2048, value=256, label="最大长度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="这个用户界面允许您输入任何文本并使用Stable Diffusion 3生成图像。"
                "您可以使用滑块自定义生成设置。"
                "试一试，看看会发生什么！",
    allow_flagging="never",
)
...
sd3_img2img_interface = gr.Interface(
    ...
    description="这个用户界面允许您输入任何文本和初始图像，使用Stable Diffusion 3生成新图像。"
                "您可以使用滑块自定义生成设置。"
                "试一试，看看会发生什么！",
    allow_flagging="never",
)
...
sd3_controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Image(label="初始图像", type="filepath"),
        gr.Dropdown(choices=["Pose", "Canny"], label="选择ControlNet模型", value="Pose"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="步骤"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="ControlNet调节比例"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="宽度"),
    ...
    description="这个用户界面允许您使用Stable Diffusion 3的ControlNet模型。"
                "您可以使用滑块自定义生成设置。"
                "试一试，看看会发生什么！",
    allow_flagging="never",
)
...
sd3_inpaint_interface = gr.Interface(
    ...
    description="这个用户界面允许您使用Stable Diffusion 3进行图像修复。"
                "您可以使用滑块自定义生成设置。"
                "试一试，看看会发生什么！",
    allow_flagging="never",
)
...
cascade_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.HTML("<h3>Stable Cascade设置</h3>"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="高度"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="先验步数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=4.0, step=0.1, label="先验引导尺度"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="解码器步数"),
        gr.Slider(minimum=0.0, maximum=30.0, value=8.0, step=0.1, label="解码器引导尺度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您输入提示并使用Stable Cascade生成图像。您可以使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
extras_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="要修改的图像", type="filepath"),
        gr.Image(label="源图像", type="filepath"),
        gr.Checkbox(label="移除背景", value=False),
        gr.Checkbox(label="启用换脸", value=False),
        gr.Checkbox(label="启用人脸修复", value=False),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(label="修改后的图像", type="filepath"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (extras)",
    description="此用户界面允许您修改图像",
    allow_flagging="never",
)
...
kandinsky_txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入你的提示词"),
        gr.Textbox(label="输入你的负面提示词", value=""),
        gr.Radio(choices=["2.1", "2.2", "3"], label="Kandinsky 版本", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步骤"),
    ...
    description="这个用户界面允许你使用Kandinsky模型生成图像。"
                "你可以在2.1、2.2和3版本之间选择，并自定义生成设置。"
                "试试看会发生什么！",
    allow_flagging="never",
)
...
flux_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Dropdown(choices=["FLUX.1-schnell", "FLUX.1-dev"], label="选择Flux模型", value="FLUX.1-schnell"),
        gr.Slider(minimum=0.0, maximum=10.0, value=0.0, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="宽度"),
    ...
    description="此用户界面允许您使用Flux模型生成图像。您可以选择Schnell和Dev模型，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
hunyuandit_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="宽度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ...
    description="此用户界面允许您使用HunyuanDiT模型生成图像。输入提示（英文或中文）并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
lumina_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="宽度"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="最大序列长度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
    ...
    description="此用户界面允许您使用Lumina-T2X模型生成图像。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
kolors_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.5, step=0.1, label="引导尺度"),
    ...
    description="此用户界面允许您使用Kolors模型生成图像。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
wurstchen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=256, maximum=2048, value=1536, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="高度"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="先验步数"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4.0, step=0.1, label="先验引导尺度"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="解码器步数"),
        gr.Slider(minimum=0.0, maximum=30.0, value=0.0, step=0.1, label="解码器引导尺度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您使用Würstchen模型生成图像。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
deepfloyd_if_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入你的负面提示词", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步骤"),
        gr.Slider(minimum=0.1, maximum=30.0, value=6, step=0.1, label="引导比例"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="高度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ...
    outputs=[
        gr.Image(type="filepath", label="生成的图像（阶段 I）"),
        gr.Image(type="filepath", label="生成的图像（阶段 II）"),
        gr.Image(type="filepath", label="生成的图像（阶段 III）"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - DeepFloyd IF (txt2img)",
    description="这个用户界面允许你使用DeepFloyd IF模型生成图像。"
                "输入提示词并自定义生成设置。"
                "该过程包括三个生成阶段，每个阶段产生的图像质量逐步提高。"
                "试试看会发生什么！",
    allow_flagging="never",
)
...
deepfloyd_if_img2img_interface = gr.Interface(
    ...
    description="这个界面允许你使用DeepFloyd IF的图像到图像管道生成图像。"
                "输入提示词，上传初始图像，并自定义生成设置。"
                "该过程包括三个生成阶段，每个阶段产生的图像质量逐步提高。"
                "试试看会发生什么！",
    allow_flagging="never",
)
...
deepfloyd_if_inpaint_interface = gr.Interface(
    ...
    description="这个界面允许你使用DeepFloyd IF进行图像修复。"
                "输入提示词，上传初始图像和蒙版图像，并自定义生成设置。"
                "该过程包括三个生成阶段，每个阶段产生的图像质量逐步提高。"
                "试试看会发生什么！",
    allow_flagging="never",
)
...
pixart_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Radio(choices=["Alpha-512", "Alpha-1024", "Sigma-512", "Sigma-1024"], label="PixArt版本", value="Alpha-512"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="宽度"),
    ...
    description="此用户界面允许您使用PixArt模型生成图像。您可以选择Alpha和Sigma版本，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
modelscope_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=1024, value=320, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=1024, value=576, step=64, label="宽度"),
        gr.Slider(minimum=16, maximum=128, value=64, step=1, label="帧数"),
        gr.Radio(choices=["mp4", "gif"], label="选择输出格式", value="mp4", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您使用ModelScope生成视频。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
zeroscope2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Video(label="要增强的视频（可选）", interactive=True),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="强度"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="步数"),
        gr.Slider(minimum=256, maximum=1280, value=576, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=1280, value=320, step=64, label="高度"),
        gr.Slider(minimum=1, maximum=100, value=36, step=1, label="帧数"),
        gr.Checkbox(label="启用视频增强", value=False),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="生成的视频"),
    ...
    description="此用户界面允许您使用ZeroScope 2模型生成和增强视频。您可以输入文本提示，上传可选的视频进行增强，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
cogvideox_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.0, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="宽度"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="帧数"),
    ...
    description="此用户界面允许您使用CogVideoX生成视频。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
latte_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="宽度"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="视频长度"),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="生成的GIF"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Latte",
    description="此用户界面允许您使用Latte生成GIF。输入提示并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
triposr_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=32, maximum=320, value=256, step=32, label="行进立方体分辨率"),
        gr.Slider(minimum=0.5, maximum=1.0, value=0.85, step=0.05, label="前景比例"),
        gr.Radio(choices=["obj", "glb"], label="选择输出格式", value="obj", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="生成的3D对象"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - TripoSR",
    description="此用户界面允许您使用TripoSR生成3D对象。上传图像并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
stablefast3d_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="输入图像", type="filepath"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=256, label="纹理分辨率"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.85, step=0.05, label="前景比例"),
        gr.Radio(choices=["none", "triangle", "quad"], label="重新网格选项", value="none"),
        gr.Radio(choices=["obj", "glb"], label="选择输出格式", value="obj", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您使用StableFast3D从图像生成3D对象。上传图像并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
shap_e_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Image(label="初始图像（可选）", type="filepath", interactive=True),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=10.0, step=0.1, label="CFG"),
        gr.Slider(minimum=64, maximum=512, value=256, step=64, label="帧大小"),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="生成的3D对象"),
    ...
    description="此用户界面允许您使用Shap-E生成3D对象。您可以输入文本提示或上传初始图像，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
sv34d_interface = gr.Interface(
    fn=generate_sv34d,
    inputs=[
        gr.File(label="输入文件（3D-U和3D-P使用图像，4D使用MP4视频）", type="filepath"),
        gr.Radio(choices=["3D-U", "3D-P", "4D"], label="版本", value="3D-U"),
        gr.Slider(minimum=0.0, maximum=90.0, value=10.0, step=0.1, label="仰角（仅适用于3D-P）"),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="生成的输出"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - SV34D",
    description="此界面允许您使用SV34D模型生成3D和4D内容。为3D-U和3D-P版本上传图像（PNG、JPG、JPEG），为4D版本上传MP4视频。选择版本并根据需要自定义设置。",
    allow_flagging="never",
)
...
zero123plus_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="输入图像", type="filepath"),
        gr.Slider(minimum=1, maximum=100, value=75, step=1, label="推理步数"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您使用Zero123Plus生成类3D图像。上传输入图像并自定义推理步数。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
stableaudio_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=12, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="音频长度（秒）"),
        gr.Slider(minimum=1, maximum=60, value=0, step=1, label="音频开始（秒）"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="波形数量"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式", value="wav", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您输入任何文本并使用StableAudio生成音频。您可以使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
audiocraft_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Audio(type="filepath", label="旋律音频（可选）", interactive=True),
        gr.Dropdown(choices=audiocraft_models_list, label="选择AudioCraft模型", value=None),
        gr.HTML("<h3>AudioCraft设置</h3>"),
        gr.Radio(choices=["musicgen", "audiogen", "magnet"], label="选择模型类型", value="musicgen"),
        gr.Slider(minimum=1, maximum=120, value=10, step=1, label="持续时间（秒）"),
        ...
        gr.Checkbox(label="启用多频带扩散", value=False),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式（仅在不使用多频带扩散时有效）", value="wav", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="生成的音频", type="filepath"),
    ...
    description="此用户界面允许您输入任何文本并使用AudioCraft生成音频。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
audioldm2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Dropdown(choices=["cvssp/audioldm2", "cvssp/audioldm2-music"], label="选择AudioLDM 2模型", value="cvssp/audioldm2"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="步数"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="长度（秒）"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="波形数量"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式", value="wav", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    ...
    description="此用户界面允许您输入任何文本并使用AudioLDM 2生成音频。您可以选择模型并使用滑块自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
demucs_interface = gr.Interface(
    fn=demucs_separate,
    inputs=[
        gr.Audio(type="filepath", label="要分离的音频文件"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="选择输出格式", value="wav", interactive=True),
    ],
    outputs=[
        gr.Audio(label="人声", type="filepath"),
        gr.Audio(label="伴奏", type="filepath"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Demucs",
    description="此用户界面允许您上传音频文件，并使用Demucs将其分离为人声和伴奏。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
gallery_interface = gr.Interface(
    ...
    inputs=[
        gr.Dropdown(label="文本文件", choices=get_output_files()[0], interactive=True),
        gr.Dropdown(label="图像文件", choices=get_output_files()[1], interactive=True),
        gr.Dropdown(label="视频文件", choices=get_output_files()[2], interactive=True),
        gr.Dropdown(label="音频文件", choices=get_output_files()[3], interactive=True),
        gr.Dropdown(label="3D模型文件", choices=get_output_files()[4], interactive=True),
    ],
    outputs=[
        gr.Textbox(label="文本"),
        gr.Image(label="图像", type="filepath"),
        gr.Video(label="视频"),
        gr.Audio(label="音频", type="filepath"),
        gr.Model3D(label="3D模型"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Gallery",
    description="此界面允许您查看输出目录中的文件",
    allow_flagging="never",
)
...
model_downloader_interface = gr.Interface(
    fn=download_model,
    inputs=[
        gr.Dropdown(choices=[None, "StarlingLM(Transformers7B)", "OpenChat(Llama7B.Q4)"], label="下载LLM模型", value=None),
        gr.Dropdown(choices=[None, "Dreamshaper8(SD1.5)", "RealisticVisionV4.0(SDXL)"], label="下载StableDiffusion模型", value=None),
    ],
    outputs=[
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - ModelDownloader",
    description="此用户界面允许您下载LLM和StableDiffusion模型",
    allow_flagging="never",
)
...
settings_interface = gr.Interface(
    ...
    inputs=[
        gr.Radio(choices=["True", "False"], label="共享模式", value="False")
    ],
    outputs=[
        gr.Textbox(label="消息", type="text")
    ],
    title="NeuroSandboxWebUI (ALPHA) - Settings",
    description="此用户界面允许您更改应用程序的设置",
    allow_flagging="never",
)
...
system_interface = gr.Interface(
    ...
    inputs=[],
    outputs=[
        gr.Textbox(label="GPU总内存"),
        gr.Textbox(label="GPU已用内存"),
        gr.Textbox(label="GPU可用内存"),
        gr.Textbox(label="GPU温度"),
        gr.Textbox(label="CPU温度"),
        gr.Textbox(label="内存总量"),
        gr.Textbox(label="内存已用"),
        gr.Textbox(label="内存可用"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - System",
    description="此界面显示系统信息",
    allow_flagging="never",
)
...
with gr.TabbedInterface(
    [
        ...
        gr.TabbedInterface(
            ...
            tab_names=["画廊", "模型下载器", "设置", "系统"]
        )
    ],
    tab_names=["文本", "图像", "视频", "3D", "音频", "界面"]
) as app:
