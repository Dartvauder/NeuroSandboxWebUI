...
kandinsky_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="输入您的提示"),
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Radio(choices=["2.1", "2.2", "3"], label="Kandinsky版本", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=1024, value=768, step=64, label="高度"),
        gr.Slider(minimum=256, maximum=1024, value=768, step=64, label="宽度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="此用户界面允许您使用Kandinsky模型生成图像。您可以选择2.1、2.2和3版本，并自定义生成设置。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
...
deepfloyd_if_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="输入您的负面提示", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="步数"),
        gr.Slider(minimum=0.1, maximum=30.0, value=6, step=0.1, label="引导尺度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="宽度"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="高度"),
        gr.Radio(choices=["png", "jpeg"], label="选择输出格式", value="png", interactive=True),
        gr.Button(value="停止生成", interactive=True, variant="stop"),
    ...
    outputs=[
        gr.Image(type="filepath", label="生成的图像（阶段I）"),
        gr.Image(type="filepath", label="生成的图像（阶段II）"),
        gr.Image(type="filepath", label="生成的图像（阶段III）"),
        gr.Textbox(label="消息", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - DeepFloyd IF",
    description="此用户界面允许您使用DeepFloyd IF模型生成图像。输入提示并自定义生成设置。生成过程包括三个阶段，每个阶段都会产生一个质量逐步提高的图像。尝试一下，看看会发生什么！",
    allow_flagging="never",
)
