...
kandinsky_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=1024, value=768, step=64, label="الارتفاع"),
        gr.Slider(minimum=256, maximum=1024, value=768, step=64, label="العرض"),
        gr.Radio(choices=["png", "jpeg"], label="اختر تنسيق الإخراج", value="png", interactive=True),
        gr.Button(value="إيقاف التوليد", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="تتيح لك واجهة المستخدم هذه توليد صور باستخدام نماذج Kandinsky. يمكنك الاختيار بين الإصدارات 2.1 و2.2 و3، وتخصيص إعدادات التوليد. جربها وانظر ماذا يحدث!",
    allow_flagging="never",
)
...
deepfloyd_if_interface = gr.Interface(
    ...
    description="تتيح لك واجهة المستخدم هذه توليد صور باستخدام نموذج DeepFloyd IF. أدخل إشارة وقم بتخصيص إعدادات التوليد. تتضمن العملية ثلاث مراحل من التوليد، كل منها ينتج صورة ذات جودة متزايدة. جربها وانظر ماذا يحدث!",
    allow_flagging="never",
)
