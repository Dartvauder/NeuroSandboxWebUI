...
chat_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje zapytanie"),
        gr.Audio(type="filepath", label="Nagraj swoje zapytanie (opcjonalnie)"),
        gr.Image(label="Prześlij swój obraz (opcjonalnie)", type="filepath"),
        gr.Dropdown(choices=llm_models_list, label="Wybierz model LLM", value=None),
        gr.Dropdown(choices=llm_lora_models_list, label="Wybierz model LoRA (opcjonalnie)", value=None),
        gr.HTML("<h3>Ustawienia LLM</h3>"),
        gr.Radio(choices=["transformers", "llama"], label="Wybierz typ modelu", value="transformers"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="Maksymalna długość (dla modeli typu transformers)"),
        gr.Slider(minimum=1, maximum=4096, value=512, step=1, label="Maksymalna liczba tokenów (dla modeli typu llama)"),
        ...
        gr.Radio(choices=["txt", "json"], label="Wybierz format historii czatu", value="txt", interactive=True),
        gr.Checkbox(label="Włącz WebSearch", value=False),
        gr.Checkbox(label="Włącz LibreTranslate", value=False),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Wybierz język docelowy", value="pl", interactive=True),
        gr.Checkbox(label="Włącz Multimodal", value=False),
        gr.Checkbox(label="Włącz TTS", value=False),
        gr.HTML("<h3>Ustawienia TTS</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="Wybierz głos", interactive=True),
        gr.Dropdown(choices=["en", "pl"], label="Wybierz język", interactive=True),
        ...
        gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="Prędkość TTS", interactive=True),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy", value="wav", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Chatbot(label="Odpowiedź tekstowa LLM", value=[]),
        gr.Audio(label="Odpowiedź dźwiękowa LLM", type="filepath"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - LLM",
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu lub dźwięku i otrzymanie wygenerowanej odpowiedzi. Możesz wybrać model LLM, "
                "awatar, głos i język dla tts z list rozwijanych. Możesz również dostosować ustawienia modelu za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
...
tts_stt_interface = gr.Interface(
    fn=generate_tts_stt,
    inputs=[
        gr.Textbox(label="Wprowadź tekst dla TTS"),
        gr.Audio(label="Nagraj dźwięk dla STT", type="filepath"),
        gr.HTML("<h3>Ustawienia TTS</h3>"),
        gr.Dropdown(choices=speaker_wavs_list, label="Wybierz głos", interactive=True),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh-cn", "ja", "hu", "ko", "hi"], label="Wybierz język", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.9, value=1.0, step=0.1, label="Temperatura TTS", interactive=True),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.9, step=0.1, label="Top P TTS", interactive=True),
        gr.Slider(minimum=0, maximum=100, value=20, step=1, label="Top K TTS", interactive=True),
        gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="Prędkość TTS", interactive=True),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy TTS", value="wav", interactive=True),
        gr.Dropdown(choices=["txt", "json"], label="Wybierz format wyjściowy STT", value="txt", interactive=True),
    ],
    outputs=[
        gr.Audio(label="Dźwięk TTS", type="filepath"),
        gr.Textbox(label="Tekst STT"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - TTS-STT",
    description="Ten interfejs użytkownika pozwala na wprowadzenie tekstu dla Text-to-Speech (CoquiTTS) i nagranie dźwięku dla Speech-to-Text (OpenAIWhisper). "
                "Dla TTS możesz wybrać głos i język oraz dostosować ustawienia generowania za pomocą suwaków. "
                "Dla STT wystarczy nagrać dźwięk, a wypowiedziany tekst zostanie wyświetlony. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
bark_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź tekst dla zapytania"),
        gr.Dropdown(choices=[None, "v2/en_speaker_1", "v2/pl_speaker_1", "v2/de_speaker_1", "v2/fr_speaker_1", "v2/es_speaker_1", "v2/hi_speaker_1", "v2/it_speaker_1", "v2/ja_speaker_1", "v2/ko_speaker_1", "v2/pt_speaker_1", "v2/zh_speaker_1", "v2/tr_speaker_1", "v2/pl_speaker_1"], label="Wybierz preset głosu", value=None),
        gr.Slider(minimum=1, maximum=1000, value=100, step=1, label="Maksymalna długość"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.4, step=0.1, label="Temperatura dokładna"),
        gr.Slider(minimum=0.1, maximum=2.0, value=0.8, step=0.1, label="Temperatura zgrubna"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy", value="wav", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Wygenerowany dźwięk", type="filepath"),
        gr.Image(label="Spektrogram Mel", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - SunoBark",
    description="Ten interfejs użytkownika pozwala na wprowadzenie tekstu i generowanie dźwięku za pomocą SunoBark. "
                "Możesz wybrać preset głosu i dostosować maksymalną długość. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
translate_interface = gr.Interface(
    fn=translate_text,
    inputs=[
        gr.Textbox(label="Wprowadź tekst do przetłumaczenia"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Wybierz język źródłowy", value="en"),
        gr.Dropdown(choices=["en", "es", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "zh", "ja", "hi"], label="Wybierz język docelowy", value="pl"),
        gr.Checkbox(label="Włącz zapisywanie historii tłumaczeń", value=False),
        gr.Radio(choices=["txt", "json"], label="Wybierz format historii tłumaczeń", value="txt", interactive=True),
        gr.File(label="Prześlij plik tekstowy (opcjonalnie)", file_count="single", interactive=True),
    ],
    outputs=[
        gr.Textbox(label="Przetłumaczony tekst"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - LibreTranslate",
    description="Ten interfejs użytkownika pozwala na wprowadzenie tekstu i przetłumaczenie go za pomocą LibreTranslate. "
                "Wybierz języki źródłowy i docelowy, a następnie kliknij Wyślij, aby uzyskać tłumaczenie. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
wav2lip_interface = gr.Interface(
    fn=generate_wav2lip,
    inputs=[
        gr.Image(label="Obraz wejściowy", type="filepath"),
        gr.Audio(label="Dźwięk wejściowy", type="filepath"),
        gr.Slider(minimum=1, maximum=60, value=30, step=1, label="FPS"),
        gr.Textbox(label="Wypełnienia", value="0 10 0 0"),
        gr.Slider(minimum=1, maximum=64, value=16, step=1, label="Rozmiar wsadu detekcji twarzy"),
        gr.Slider(minimum=1, maximum=512, value=128, step=1, label="Rozmiar wsadu Wav2Lip"),
        gr.Slider(minimum=1, maximum=4, value=1, step=1, label="Współczynnik zmiany rozmiaru"),
        gr.Textbox(label="Przycięcie", value="0 -1 0 -1"),
    ],
    outputs=[
        gr.Video(label="Wygenerowana synchronizacja ust"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Wav2Lip",
    description="Ten interfejs użytkownika pozwala na generowanie filmów z mówiącą głową poprzez połączenie obrazu i pliku dźwiękowego za pomocą Wav2Lip. "
                "Prześlij obraz i plik dźwiękowy, a następnie kliknij Generuj, aby stworzyć film z mówiącą głową. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion", value=None),
        gr.Dropdown(choices=vae_models_list, label="Wybierz model VAE (opcjonalnie)", value=None),
        gr.Dropdown(choices=lora_models_list, label="Wybierz modele LORA (opcjonalnie)", value=None, multiselect=True),
        gr.Dropdown(choices=textual_inversion_models_list, label="Wybierz modele Embedding (opcjonalnie)", value=None, multiselect=True),
        gr.HTML("<h3>Ustawienia StableDiffusion</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Wybierz typ modelu", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
                    label="Wybierz sampler", value="euler_ancestral"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=1, maximum=4, value=1, step=1, label="Clip skip"),
        gr.Checkbox(label="Włącz FreeU", value=False),
        gr.Checkbox(label="Włącz Tiled VAE", value=False),
        gr.Checkbox(label="Włącz Upscale", value=False),
        gr.Radio(choices=["x2", "x4"], label="Rozmiar upscale", value="x2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki upscale"),
        gr.Slider(minimum=1.0, maximum=30.0, value=6, step=0.1, label="Upscale CFG"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Wygenerowany obraz"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (txt2img)",
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i generowanie obrazów za pomocą StableDiffusion. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
img2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="Siła"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion", value=None),
        gr.Dropdown(choices=vae_models_list, label="Wybierz model VAE (opcjonalnie)", value=None),
        gr.HTML("<h3>Ustawienia StableDiffusion</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Wybierz typ modelu", value="SD"),
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i obrazu w celu wygenerowania nowych obrazów za pomocą StableDiffusion. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
depth2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.HTML("<h3>Ustawienia StableDiffusion</h3>"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.7, step=0.01, label="Siła"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia, obrazu początkowego w celu wygenerowania obrazów uwzględniających głębię za pomocą StableDiffusion. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
pix2pix_interface = gr.Interface(
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia i obrazu początkowego w celu wygenerowania nowych obrazów za pomocą Pix2Pix. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.Radio(choices=["SD", "SDXL"], label="Wybierz typ modelu", value="SD"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion", value=None),
        gr.Dropdown(choices=controlnet_models_list, label="Wybierz model ControlNet", value=None),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="Skala uwarunkowania ControlNet"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Wygenerowany obraz"),
        gr.Image(type="filepath", label="Obraz kontrolny ControlNet"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (controlnet)",
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modeli ControlNet. "
                "Prześlij obraz początkowy, wprowadź polecenie, wybierz model Stable Diffusion i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
latent_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz do powiększenia", type="filepath"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="CFG"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Powiększony obraz"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (upscale-latent)",
    description="Ten interfejs użytkownika pozwala na przesłanie obrazu i powiększenie go w przestrzeni ukrytej",
    allow_flagging="never",
)
...
realesrgan_upscale_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz do powiększenia", type="filepath"),
        gr.Slider(minimum=0.1, maximum=8, value=4, step=0.1, label="Współczynnik powiększenia"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na przesłanie obrazu i powiększenie go za pomocą Real-ESRGAN",
    allow_flagging="never",
)
...
inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.ImageEditor(label="Obraz maski", type="filepath"),
        gr.Slider(minimum=0, maximum=100, value=0, step=1, label="Współczynnik rozmycia maski"),
        gr.Dropdown(choices=inpaint_models_list, label="Wybierz model Inpaint", value=None),
        gr.Dropdown(choices=vae_models_list, label="Wybierz model VAE (opcjonalnie)", value=None),
        gr.HTML("<h3>Ustawienia StableDiffusion</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Wybierz typ modelu", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    outputs=[
        gr.Image(type="filepath", label="Obraz po inpaintingu"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (inpaint)",
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia, obrazu początkowego i obrazu maski do inpaintingu za pomocą StableDiffusion. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
gligen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Textbox(label="Wprowadź frazy GLIGEN", value=""),
        gr.Textbox(label="Wprowadź ramki GLIGEN", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion", value=None),
        gr.HTML("<h3>Ustawienia StableDiffusion</h3>"),
        gr.Radio(choices=["SD", "SD2", "SDXL"], label="Wybierz typ modelu", value="SD"),
        gr.Dropdown(choices=["euler_ancestral", "euler", "lms", "heun", "dpm", "dpm_solver", "dpm_solver++"],
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą Stable Diffusion i wstawianie obiektów za pomocą GLIGEN. "
                "Wybierz model Stable Diffusion, dostosuj ustawienia generowania, wprowadź polecenie, frazy GLIGEN i ramki ograniczające. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
animatediff_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Początkowy GIF", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="Siła"),
        gr.Radio(choices=["sd", "sdxl"], label="Wybierz typ modelu", value="sd"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion (tylko SD1.5)", value=None),
        gr.Dropdown(choices=[None, "zoom-in", "zoom-out", "tilt-up", "tilt-down", "pan-right", "pan-left"], label="Wybierz Motion LORA", value=None, multiselect=True),
        gr.Slider(minimum=1, maximum=200, value=20, step=1, label="Klatki"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Wysokość"),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(label="Wygenerowany GIF", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (animatediff)",
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia i generowanie animowanych GIF-ów za pomocą AnimateDiff. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
video_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.Radio(choices=["mp4", "gif"], label="Wybierz format wyjściowy", value="mp4", interactive=True),
        gr.HTML("<h3>Ustawienia SVD (mp4)</h3>"),
        gr.Slider(minimum=0, maximum=360, value=180, step=1, label="ID Motion Bucket"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.1, step=0.01, label="Siła augmentacji szumu"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="FPS"),
        gr.Slider(minimum=2, maximum=120, value=25, step=1, label="Klatki"),
        gr.Slider(minimum=1, maximum=32, value=8, step=1, label="Rozmiar chunka dekodowania"),
        gr.HTML("<h3>Ustawienia I2VGen-xl (gif)</h3>"),
        gr.Textbox(label="Polecenie", value=""),
        gr.Textbox(label="Negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=9.0, step=0.1, label="CFG"),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Wygenerowane wideo"),
        gr.Image(label="Wygenerowany GIF", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (video)",
    description="Ten interfejs użytkownika pozwala na wprowadzenie obrazu początkowego i wygenerowanie wideo za pomocą StableVideoDiffusion(mp4) i I2VGen-xl(gif). "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
ldm3d_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8, step=0.1, label="Skala Guidance"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Wygenerowany obraz RGB"),
        gr.Image(type="filepath", label="Wygenerowany obraz głębi"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (LDM3D)",
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia i generowanie obrazów RGB i głębi za pomocą LDM3D. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
sd3_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Wysokość"),
        gr.Slider(minimum=64, maximum=2048, value=256, label="Maksymalna długość"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i generowanie obrazów za pomocą Stable Diffusion 3. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
sd3_img2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Siła"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Wysokość"),
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i obrazu początkowego do generowania nowych obrazów za pomocą Stable Diffusion 3. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
sd3_controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.Dropdown(choices=["Pose", "Canny"], label="Wybierz model ControlNet", value="Pose"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="Skala uwarunkowania ControlNet"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Szerokość"),
    ...
    description="Ten interfejs użytkownika pozwala na używanie modeli ControlNet z Stable Diffusion 3. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
sd3_inpaint_interface = gr.Interface(
    ...
    description="Ten interfejs użytkownika pozwala na wykonywanie inpaintingu za pomocą Stable Diffusion 3. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
cascade_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.HTML("<h3>Ustawienia Stable Cascade</h3>"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=64, label="Wysokość"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki Prior"),
        gr.Slider(minimum=1.0, maximum=30.0, value=4.0, step=0.1, label="Skala Guidance Prior"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="Kroki Decoder"),
        gr.Slider(minimum=0.0, maximum=30.0, value=8.0, step=0.1, label="Skala Guidance Decoder"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie polecenia i generowanie obrazów za pomocą Stable Cascade. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
instantid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Obraz twarzy", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion XL", value=None),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Skala uwarunkowania ControlNet"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Skala IP-Adapter"),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Skala Guidance (CFG)"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą InstantID. "
                "Prześlij obraz twarzy, wprowadź polecenie, wybierz model Stable Diffusion i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
photomaker_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.File(label="Prześlij obrazy wejściowe", file_count="multiple", type="filepath"),
        gr.Textbox(label="Wprowadź słowo wyzwalające", value="img"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion XL", value=None),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Skala Guidance (CFG)"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą PhotoMaker. "
                "Prześlij obrazy wejściowe, wprowadź polecenie, wybierz model bazowy i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
ip_adapter_faceid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Image(label="Prześlij obraz twarzy", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Wybierz model StableDiffusion", value=None),
        gr.Dropdown(
            choices=[
            ...
            label="Wybierz wersję IP-Adapter-FaceID",
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą IP-Adapter-FaceID. "
                "Prześlij obraz twarzy, wprowadź polecenie, wybierz model bazowy i wersję IP-Adapter, oraz dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
extras_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz do modyfikacji", type="filepath"),
        gr.Image(label="Obraz źródłowy", type="filepath"),
        gr.Checkbox(label="Usuń tło", value=False),
        gr.Checkbox(label="Włącz zamianę twarzy", value=False),
        gr.Checkbox(label="Włącz odnawianie twarzy", value=False),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(label="Zmodyfikowany obraz", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableDiffusion (dodatki)",
    description="Ten interfejs użytkownika pozwala na modyfikację obrazu",
    allow_flagging="never",
)
...
kandinsky_txt2img_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Radio(choices=["2.1", "2.2", "3"], label="Wersja Kandinsky", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Szerokość"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modeli Kandinsky. "
                "Możesz wybrać między wersjami 2.1, 2.2 i 3 oraz dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
kandinsky_inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="Obraz początkowy", type="filepath"),
        gr.ImageEditor(label="Obraz maski", type="filepath"),
        gr.Radio(choices=["2.1", "2.2"], label="Wersja Kandinsky", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="Siła"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
    ...
    description="Ten interfejs użytkownika pozwala na wykonywanie inpaintingu za pomocą modeli Kandinsky. "
                "Możesz wybrać między wersjami 2.1 i 2.2 oraz dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
flux_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Dropdown(choices=["FLUX.1-schnell", "FLUX.1-dev"], label="Wybierz model Flux", value="FLUX.1-schnell"),
        gr.Slider(minimum=0.0, maximum=10.0, value=0.0, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Szerokość"),
        gr.Slider(minimum=1, maximum=100, value=10, step=1, label="Kroki"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksymalna długość sekwencji"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modeli Flux. "
                "Możesz wybrać między modelami Schnell i Dev oraz dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
hunyuandit_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Szerokość"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu HunyuanDiT. "
                "Wprowadź polecenie (w języku angielskim lub chińskim) i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
lumina_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="Szerokość"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksymalna długość sekwencji"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu Lumina-T2X. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
kolors_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.5, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=1, maximum=100, value=25, step=1, label="Kroki"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksymalna długość sekwencji"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu Kolors. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
auraflow_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=25, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="Maksymalna długość sekwencji"),
        gr.Checkbox(label="Włącz AuraSR", value=False),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu AuraFlow. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Możesz również włączyć AuraSR dla 4-krotnego powiększenia wygenerowanego obrazu. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
wurstchen_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=256, maximum=2048, value=1536, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="Wysokość"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki Prior"),
        gr.Slider(minimum=0.1, maximum=30.0, value=4.0, step=0.1, label="Skala Guidance Prior"),
        gr.Slider(minimum=1, maximum=100, value=20, step=1, label="Kroki Decoder"),
        gr.Slider(minimum=0.0, maximum=30.0, value=0.0, step=0.1, label="Skala Guidance Decoder"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu Würstchen. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
deepfloyd_if_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=30.0, value=6, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ...
    outputs=[
        gr.Image(type="filepath", label="Wygenerowany obraz (Etap I)"),
        gr.Image(type="filepath", label="Wygenerowany obraz (Etap II)"),
        gr.Image(type="filepath", label="Wygenerowany obraz (Etap III)"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - DeepFloyd IF (txt2img)",
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modelu DeepFloyd IF. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Proces obejmuje trzy etapy generowania, każdy produkujący obraz o rosnącej jakości. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
deepfloyd_if_img2img_interface = gr.Interface(
    ...
    description="Ten interfejs pozwala na generowanie obrazów za pomocą potoku obraz-do-obrazu DeepFloyd IF. "
                "Wprowadź polecenie, prześlij obraz początkowy i dostosuj ustawienia generowania. "
                "Proces obejmuje trzy etapy generowania, każdy produkujący obraz o rosnącej jakości. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
deepfloyd_if_inpaint_interface = gr.Interface(
    ...
    description="Ten interfejs pozwala na wykonywanie inpaintingu za pomocą DeepFloyd IF. "
                "Wprowadź polecenie, prześlij obraz początkowy i obraz maski, oraz dostosuj ustawienia generowania. "
                "Proces obejmuje trzy etapy generowania, każdy produkujący obraz o rosnącej jakości. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
pixart_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Radio(choices=["Alpha-512", "Alpha-1024", "Sigma-512", "Sigma-1024"], label="Wersja PixArt", value="Alpha-512"),
        gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=30.0, value=7.5, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="Szerokość"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów za pomocą modeli PixArt. "
                "Możesz wybrać między wersjami Alpha i Sigma oraz dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
modelscope_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=1024, value=320, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=1024, value=576, step=64, label="Szerokość"),
        gr.Slider(minimum=16, maximum=128, value=64, step=1, label="Liczba klatek"),
        gr.Radio(choices=["mp4", "gif"], label="Wybierz format wyjściowy", value="mp4", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie wideo za pomocą ModelScope. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
zeroscope2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Video(label="Wideo do ulepszenia (opcjonalne)", interactive=True),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="Siła"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="Kroki"),
        gr.Slider(minimum=256, maximum=1280, value=576, step=64, label="Szerokość"),
        gr.Slider(minimum=256, maximum=1280, value=320, step=64, label="Wysokość"),
        gr.Slider(minimum=1, maximum=100, value=36, step=1, label="Klatki"),
        gr.Checkbox(label="Włącz ulepszanie wideo", value=False),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Wygenerowane wideo"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie i ulepszanie wideo za pomocą modeli ZeroScope 2. "
                "Możesz wprowadzić polecenie tekstowe, przesłać opcjonalne wideo do ulepszenia i dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
cogvideox_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=20.0, value=6.0, step=0.1, label="Skala Guidance"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="Liczba klatek"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie wideo za pomocą CogVideoX. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
latte_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Wysokość"),
        gr.Slider(minimum=256, maximum=1024, value=512, step=64, label="Szerokość"),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="Długość wideo"),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="Wygenerowany GIF"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Latte",
    description="Ten interfejs użytkownika pozwala na generowanie GIF-ów za pomocą Latte. "
                "Wprowadź polecenie i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
triposr_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz wejściowy", type="pil"),
        gr.Slider(minimum=32, maximum=320, value=256, step=32, label="Rozdzielczość Marching Cubes"),
        gr.Slider(minimum=0.5, maximum=1.0, value=0.85, step=0.05, label="Współczynnik pierwszego planu"),
        gr.Radio(choices=["obj", "glb"], label="Wybierz format wyjściowy", value="obj", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="Wygenerowany obiekt 3D"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - TripoSR",
    description="Ten interfejs użytkownika pozwala na generowanie obiektów 3D za pomocą TripoSR. "
                "Prześlij obraz i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
stablefast3d_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz wejściowy", type="filepath"),
        gr.Slider(minimum=256, maximum=4096, value=1024, step=256, label="Rozdzielczość tekstury"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.85, step=0.05, label="Współczynnik pierwszego planu"),
        gr.Radio(choices=["none", "triangle", "quad"], label="Opcja ponownego tworzenia siatki", value="none"),
        gr.Radio(choices=["obj", "glb"], label="Wybierz format wyjściowy", value="obj", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obiektów 3D z obrazów za pomocą StableFast3D. "
                "Prześlij obraz i dostosuj ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
shap_e_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Image(label="Obraz początkowy (opcjonalnie)", type="filepath", interactive=True),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="Kroki"),
        gr.Slider(minimum=1.0, maximum=30.0, value=10.0, step=0.1, label="CFG"),
        gr.Slider(minimum=64, maximum=512, value=256, step=64, label="Rozmiar klatki"),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Model3D(label="Wygenerowany obiekt 3D"),
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obiektów 3D za pomocą Shap-E. "
                "Możesz wprowadzić polecenie tekstowe lub przesłać obraz początkowy, a także dostosować ustawienia generowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
sv34d_interface = gr.Interface(
    fn=generate_sv34d,
    inputs=[
        gr.File(label="Plik wejściowy (Obraz dla 3D-U i 3D-P, wideo MP4 dla 4D)", type="filepath"),
        gr.Radio(choices=["3D-U", "3D-P", "4D"], label="Wersja", value="3D-U"),
        gr.Slider(minimum=0.0, maximum=90.0, value=10.0, step=0.1, label="Stopień elewacji (tylko dla 3D-P)"),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Video(label="Wygenerowane wyjście"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - SV34D",
    description="Ten interfejs pozwala na generowanie treści 3D i 4D za pomocą modeli SV34D. "
                "Prześlij obraz (PNG, JPG, JPEG) dla wersji 3D-U i 3D-P lub wideo MP4 dla wersji 4D. "
                "Wybierz wersję i dostosuj ustawienia według potrzeb.",
    allow_flagging="never",
)
...
zero123plus_interface = gr.Interface(
    ...
    inputs=[
        gr.Image(label="Obraz wejściowy", type="filepath"),
        gr.Slider(minimum=1, maximum=100, value=75, step=1, label="Kroki wnioskowania"),
        gr.Radio(choices=["png", "jpeg"], label="Wybierz format wyjściowy", value="png", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="Ten interfejs użytkownika pozwala na generowanie obrazów 3D-podobnych za pomocą Zero123Plus. "
                "Prześlij obraz wejściowy i dostosuj liczbę kroków wnioskowania. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
stableaudio_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="Kroki"),
        gr.Slider(minimum=0.1, maximum=12, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="Długość audio (sekundy)"),
        gr.Slider(minimum=1, maximum=60, value=0, step=1, label="Początek audio (sekundy)"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="Liczba przebiegów"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy", value="wav", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Wygenerowane audio", type="filepath"),
        gr.Image(label="Spektrogram Mel", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - StableAudio",
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i generowanie dźwięku za pomocą StableAudio. "
                "Możesz dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
audiocraft_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Audio(type="filepath", label="Audio melodii (opcjonalnie)", interactive=True),
        gr.Dropdown(choices=audiocraft_models_list, label="Wybierz model AudioCraft", value=None),
        gr.HTML("<h3>Ustawienia AudioCraft</h3>"),
        gr.Radio(choices=["musicgen", "audiogen", "magnet"], label="Wybierz typ modelu", value="musicgen"),
        gr.Slider(minimum=1, maximum=120, value=10, step=1, label="Czas trwania (sekundy)"),
        ...
        gr.Checkbox(label="Włącz dyfuzję wielopasmową", value=False),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy (Działa tylko bez dyfuzji wielopasmowej)", value="wav", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Audio(label="Wygenerowane audio", type="filepath"),
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i generowanie dźwięku za pomocą AudioCraft. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
audioldm2_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="Wprowadź swoje polecenie"),
        gr.Textbox(label="Wprowadź swoje negatywne polecenie", value=""),
        gr.Dropdown(choices=["cvssp/audioldm2", "cvssp/audioldm2-music"], label="Wybierz model AudioLDM 2", value="cvssp/audioldm2"),
        gr.Slider(minimum=1, maximum=1000, value=200, step=1, label="Kroki"),
        gr.Slider(minimum=1, maximum=60, value=10, step=1, label="Długość (sekundy)"),
        gr.Slider(minimum=1, maximum=10, value=3, step=1, label="Liczba przebiegów"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy", value="wav", interactive=True),
        gr.Button(value="Zatrzymaj generowanie", interactive=True, variant="stop"),
    ],
    ...
    description="Ten interfejs użytkownika pozwala na wprowadzenie dowolnego tekstu i generowanie dźwięku za pomocą AudioLDM 2. "
                "Możesz wybrać model i dostosować ustawienia generowania za pomocą suwaków. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
demucs_interface = gr.Interface(
    fn=demucs_separate,
    inputs=[
        gr.Audio(type="filepath", label="Plik audio do separacji"),
        gr.Radio(choices=["wav", "mp3", "ogg"], label="Wybierz format wyjściowy", value="wav", interactive=True),
    ],
    outputs=[
        gr.Audio(label="Wokal", type="filepath"),
        gr.Audio(label="Instrumentalny", type="filepath"),
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Demucs",
    description="Ten interfejs użytkownika pozwala na przesłanie pliku audio i separację go na wokal i instrumentalny za pomocą Demucs. "
                "Wypróbuj i zobacz, co się stanie!",
    allow_flagging="never",
)
...
gallery_interface = gr.Interface(
    ...
    inputs=[
        gr.Dropdown(label="Pliki tekstowe", choices=get_output_files()[0], interactive=True),
        gr.Dropdown(label="Pliki obrazów", choices=get_output_files()[1], interactive=True),
        gr.Dropdown(label="Pliki wideo", choices=get_output_files()[2], interactive=True),
        gr.Dropdown(label="Pliki audio", choices=get_output_files()[3], interactive=True),
        gr.Dropdown(label="Pliki modeli 3D", choices=get_output_files()[4], interactive=True),
    ],
    outputs=[
        gr.Textbox(label="Tekst"),
        gr.Image(label="Obraz", type="filepath"),
        gr.Video(label="Wideo"),
        gr.Audio(label="Audio", type="filepath"),
        gr.Model3D(label="Model 3D"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - Galeria",
    description="Ten interfejs pozwala na przeglądanie plików z katalogu wyjściowego",
    allow_flagging="never",
)
...
model_downloader_interface = gr.Interface(
    fn=download_model,
    inputs=[
        gr.Dropdown(choices=[None, "StarlingLM(Transformers7B)", "OpenChat(Llama7B.Q4)"], label="Pobierz model LLM", value=None),
        gr.Dropdown(choices=[None, "Dreamshaper8(SD1.5)", "RealisticVisionV4.0(SDXL)"], label="Pobierz model StableDiffusion", value=None),
    ],
    outputs=[
        gr.Textbox(label="Wiadomość", type="text"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - ModelDownloader",
    description="Ten interfejs użytkownika pozwala na pobieranie modeli LLM i StableDiffusion",
    allow_flagging="never",
)
...
settings_interface = gr.Interface(
    ...
    inputs=[
        gr.Radio(choices=["True", "False"], label="Tryb udostępniania", value="False")
    ],
    outputs=[
        gr.Textbox(label="Wiadomość", type="text")
    ],
    title="NeuroSandboxWebUI (ALPHA) - Ustawienia",
    description="Ten interfejs użytkownika pozwala na zmianę ustawień aplikacji",
    allow_flagging="never",
)
...
system_interface = gr.Interface(
    ...
    inputs=[],
    outputs=[
        gr.Textbox(label="Całkowita pamięć GPU"),
        gr.Textbox(label="Użyta pamięć GPU"),
        gr.Textbox(label="Wolna pamięć GPU"),
        gr.Textbox(label="Temperatura GPU"),
        gr.Textbox(label="Temperatura CPU"),
        gr.Textbox(label="Całkowita pamięć RAM"),
        gr.Textbox(label="Użyta pamięć RAM"),
        gr.Textbox(label="Wolna pamięć RAM"),
    ],
    title="NeuroSandboxWebUI (ALPHA) - System",
    description="Ten interfejs wyświetla informacje o systemie",
    allow_flagging="never",
)
...
with gr.TabbedInterface(
    [
        ...
        gr.TabbedInterface(
            [
                gr.TabbedInterface(
                    ...
                    tab_names=["txt2img", "img2img", "depth2img", "pix2pix", "controlnet", "upscale(latent)", "upscale(Real-ESRGAN)", "inpaint", "gligen", "animatediff", "video", "ldm3d", "sd3", "cascade", "adaptery", "dodatki"]
                ),
        ...
        gr.TabbedInterface(
            ...
            tab_names=["Galeria", "PobieraczModeli", "Ustawienia", "System"]
        )
    ],
    tab_names=["Tekst", "Obraz", "Wideo", "3D", "Audio", "Interfejs"]
) as app:
    ...
    close_button = gr.Button("Zamknij terminal")
    close_button.click(close_terminal, [], [], queue=False)

    folder_button = gr.Button("Wyjścia")
