...
bark_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(choices=[None, "v2/en_speaker_1", "v2/ru_speaker_1", "v2/de_speaker_1", "v2/fr_speaker_1", "v2/es_speaker_1", "v2/hi_speaker_1", "v2/it_speaker_1", "v2/ja_speaker_1", "v2/ko_speaker_1", "v2/pt_speaker_1", "v2/zh_speaker_1", "v2/tr_speaker_1", "v2/pl_speaker_1"], label="音声プリセットを選択", value=None),
    ...
    outputs=[
        ...
        gr.Image(label="メルスペクトログラム", type="filepath"),
...
controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="ネガティブプロンプトを入力", value=""),
        gr.Image(label="初期画像", type="filepath"),
        gr.Radio(choices=["SD", "SDXL"], label="モデルタイプを選択", value="SD"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusionモデルを選択", value=None),
        ...
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="幅"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="高さ"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="ControlNetコンディショニングスケール"),
        gr.Radio(choices=["png", "jpeg"], label="出力形式を選択", value="png", interactive=True),
        gr.Button(value="生成を停止", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="生成された画像"),
        gr.Image(type="filepath", label="ControlNet制御画像"),
        gr.Textbox(label="メッセージ", type="text"),
    ],
...
sd3_txt2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="ステップ数"),
        ...
        gr.Radio(choices=["png", "jpeg"], label="出力フォーマットを選択", value="png", interactive=True),
        gr.Button(value="生成を停止", interactive=True, variant="stop"),
    ],
    outputs=[
        gr.Image(type="filepath", label="生成された画像"),
    ...
    description="このユーザーインターフェースでは、任意のテキストを入力し、Stable Diffusion 3を使用して画像を生成することができます。"
                "スライダーを使用して生成設定をカスタマイズできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
sd3_img2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.Image(label="初期画像", type="filepath"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="強度"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="ステップ数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="幅"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="高さ"),
    ...
    description="このユーザーインターフェースでは、任意のテキストと初期画像を入力し、Stable Diffusion 3を使用して新しい画像を生成することができます。"
                "スライダーを使用して生成設定をカスタマイズできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
sd3_controlnet_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.Image(label="初期画像", type="filepath"),
        gr.Dropdown(choices=["Pose", "Canny"], label="ControlNetモデルを選択", value="Pose"),
        gr.Slider(minimum=1, maximum=100, value=40, step=1, label="ステップ数"),
        gr.Slider(minimum=1.0, maximum=30.0, value=8.0, step=0.1, label="CFG"),
        gr.Slider(minimum=0.1, maximum=1.0, value=0.5, step=0.1, label="ControlNetコンディショニングスケール"),
        gr.Slider(minimum=256, maximum=2048, value=1024, step=64, label="幅"),
    ...
    description="このユーザーインターフェースでは、Stable Diffusion 3でControlNetモデルを使用することができます。"
                "スライダーを使用して生成設定をカスタマイズできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
sd3_inpaint_interface = gr.Interface(
    ...
    description="このユーザーインターフェースでは、Stable Diffusion 3を使用してインペインティングを行うことができます。"
                "スライダーを使用して生成設定をカスタマイズできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
instantid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="プロンプトを入力してください"),
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.Image(label="顔画像", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion XLモデルを選択", value=None),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="ControlNetコンディショニングスケール"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="IP-Adapterスケール"),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="ステップ数"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="ガイダンススケール（CFG）"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="幅"),
    ...
    description="このユーザーインターフェースでは、InstantIDを使用して画像を生成できます。"
                "顔画像をアップロードし、プロンプトを入力し、Stable Diffusionモデルを選択して、生成設定をカスタマイズしてください。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
photomaker_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="プロンプトを入力してください"),
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.File(label="入力画像をアップロード", file_count="multiple", type="filepath"),
        gr.Textbox(label="トリガーワードを入力", value="img"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusion XLモデルを選択", value=None),
        gr.Slider(minimum=1, maximum=150, value=30, step=1, label="ステップ数"),
        gr.Slider(minimum=1.0, maximum=20.0, value=7.5, step=0.1, label="ガイダンススケール（CFG）"),
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="幅"),
    ...
    description="このユーザーインターフェースでは、PhotoMakerを使用して画像を生成できます。"
                "入力画像をアップロードし、プロンプトを入力し、ベースモデルを選択して、生成設定をカスタマイズしてください。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
ip_adapter_faceid_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label="プロンプトを入力してください"),
        gr.Textbox(label="ネガティブプロンプトを入力してください", value=""),
        gr.Image(label="顔画像をアップロード", type="filepath"),
        gr.Dropdown(choices=stable_diffusion_models_list, label="StableDiffusionモデルを選択", value=None),
        gr.Dropdown(
            choices=[
            ...
            label="IP-Adapter-FaceIDバージョンを選択",
    ...
    description="このユーザーインターフェースでは、IP-Adapter-FaceIDを使用して画像を生成できます。"
                "顔画像をアップロードし、プロンプトを入力し、ベースモデルとIP-Adapterバージョンを選択して、生成設定をカスタマイズしてください。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
kandinsky_txt2img_interface = gr.Interface(
    ...
    description="このユーザーインターフェースを使用すると、Kandinskyモデルを使用して画像を生成できます。"
                "バージョン2.1、2.2、3の中から選択し、生成設定をカスタマイズできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
kandinsky_inpaint_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(label="初期画像", type="filepath"),
        gr.ImageEditor(label="マスク画像", type="filepath"),
        gr.Radio(choices=["2.1", "2.2"], label="Kandinskyバージョン", value="2.2"),
        gr.Slider(minimum=1, maximum=100, value=50, step=1, label="ステップ数"),
        gr.Slider(minimum=0.1, maximum=20, value=4, step=0.1, label="CFG"),
        gr.Slider(minimum=0.0, maximum=1.0, value=0.8, step=0.01, label="強度"),
        gr.Slider(minimum=256, maximum=2048, value=768, step=64, label="高さ"),
    ...
    description="このユーザーインターフェースでは、Kandinskyモデルを使用してインペインティングを行うことができます。"
                "バージョン2.1と2.2から選択し、生成設定をカスタマイズすることができます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
auraflow_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=256, maximum=2048, value=512, step=64, label="幅"),
        gr.Slider(minimum=1, maximum=1024, value=256, step=1, label="最大シーケンス長"),
        gr.Checkbox(label="AuraSRを有効にする", value=False),
        gr.Radio(choices=["png", "jpeg"], label="出力フォーマットを選択", value="png", interactive=True),
        gr.Button(value="生成を停止", interactive=True, variant="stop"),
    ],
    outputs=[
    ...
    description="このユーザーインターフェースでは、AuraFlowモデルを使用して画像を生成できます。"
                "プロンプトを入力し、生成設定をカスタマイズしてください。"
                "また、生成された画像を4倍に拡大するAuraSRを有効にすることもできます。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
deepfloyd_if_txt2img_interface = gr.Interface(
    ...
    description="このユーザーインターフェースを使用すると、DeepFloyd IFモデルを使用して画像を生成できます。"
                "プロンプトを入力し、生成設定をカスタマイズしてください。"
                "プロセスには3つの生成ステージが含まれ、それぞれがより高品質な画像を生成します。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
deepfloyd_if_img2img_interface = gr.Interface(
    ...
    description="このインターフェースを使用すると、DeepFloyd IFの画像から画像へのパイプラインを使用して画像を生成できます。"
                "プロンプトを入力し、初期画像をアップロードし、生成設定をカスタマイズしてください。"
                "プロセスには3つの生成ステージが含まれ、それぞれがより高品質な画像を生成します。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
...
deepfloyd_if_inpaint_interface = gr.Interface(
    ...
    description="このインターフェースを使用すると、DeepFloyd IFを使用してインペインティングを実行できます。"
                "プロンプトを入力し、初期画像とマスク画像をアップロードし、生成設定をカスタマイズしてください。"
                "プロセスには3つの生成ステージが含まれ、それぞれがより高品質な画像を生成します。"
                "試してみて、何が起こるか見てみましょう！",
    allow_flagging="never",
)
