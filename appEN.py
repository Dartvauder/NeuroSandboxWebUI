...
def generate_wav2lip(image_path, audio_path):
    ...
    try:
        ...
        command = f"python {os.path.join(wav2lip_path, 'inference.py')} --checkpoint_path {checkpoint_path} --face {image_path} --audio {audio_path} --outfile {output_path} --fps {30} --pads [0, 10, 0, 0] --face_det_batch_size {16} --wav2lip_batch_size {128} --resize_factor {1} --crop [0, -1, 0, -1] --box [-1, -1, -1, -1]"
