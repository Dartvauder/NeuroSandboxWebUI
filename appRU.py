...
animatediff_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Enter your negative prompt", value=""),
        gr.Dropdown(choices=stable_diffusion_models_list, label="Select StableDiffusion model", value=None),
        gr.Slider(minimum=1, maximum=100, value=16, step=1, label="Frames"),
        gr.Slider(minimum=1, maximum=100, value=25, step=1, label="Steps"),
        gr.Slider(minimum=1.0, maximum=30.0, value=7.5, step=0.1, label="Guidance Scale"),
        gr.Slider(minimum=256, maximum=1280, value=512, step=64, label="Width"),
        gr.Slider(minimum=256, maximum=1280, value=512, step=64, label="Height"),
        gr.Button(value="Stop generation", interactive=True, variant="stop"),
    ],
    outputs=[
