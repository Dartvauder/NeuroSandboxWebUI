...
2) Выберите тип модели (`transformers` или `llamacpp`)
3) Настройте модель по нужным вам параметрам
4) Выберите свою модель в раскрывающемся списке `LLM`
5) Введите (или произнесите) промт
6) Нажмите кнопку `Generate` чтобы получить текстовый и аудио ответ
...
#### Голос должен быть предварительно обработан (22050 кГц, монозвук, WAV), аватар предпочтительно должен быть `PNG` или `JPG`. При включении `TTS` требуется выбор языка и голоса, иначе будут ошибки
...
2) Впишите ваш текстовый запрос
3) Выберите модель из выпадающего списка
4) Настройте модель по нужным вам параметрам
5) Включите `Stable Diffusion`
6) Нажмите кнопку `generate`, чтобы получить изображение
